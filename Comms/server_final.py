        ...
        if all(len(v) == WINDOW for v in buf[1].values()):
            with mpu1_lock:
                window = np.column_stack([
                    np.array(buf[1]["yaw"],    dtype=np.float32),
                    np.array(buf[1]["pitch"],  dtype=np.float32),
                    np.array(buf[1]["roll"],   dtype=np.float32),
                    np.array(buf[1]["accelx"], dtype=np.float32),
                    np.array(buf[1]["accely"], dtype=np.float32),
                    np.array(buf[1]["accelz"], dtype=np.float32),
                ])  # (60,6)
            with AIModelUsed:
                out = ai_infer(window)
...
def AILoopPlayer2():
    while True:
        ...
        if all(len(v) == WINDOW for v in buf[2].values()):
            with mpu2_lock:
                window = np.column_stack([
                    np.array(buf[2]["yaw"],    dtype=np.float32),
                    np.array(buf[2]["pitch"],  dtype=np.float32),
                    np.array(buf[2]["roll"],   dtype=np.float32),
                    np.array(buf[2]["accelx"], dtype=np.float32),
                    np.array(buf[2]["accely"], dtype=np.float32),
                    np.array(buf[2]["accelz"], dtype=np.float32),
                ])  # (60,6)
            with AIModelUsed:
                out = ai_infer(window)
